...
def main():
    ...
    with open(out,"wb") as w:
        for ch in man["chunks"]:
            p=d/ch["name"]; raw=p.read_bytes(); nonce,ct=raw[:12], raw[12:]
            if hashlib.sha256(ct).hexdigest()!=ch["sha256"]:
                print(f"Hash mismatch: {p}", file=sys.stderr); sys.exit(3)
            pt=AESGCM(key).decrypt(nonce, ct, man["original_filename"].encode()); w.write(pt)
    ok_size=out.stat().st_size==man["original_size"]; ok_hash=sha256_path(out)==man["original_sha256"]
    print(f"Wrote {out} ({out.stat().st_size} bytes)"); print(f"Size match: {ok_size}, SHA256 match: {ok_hash}")
    if not (ok_size and ok_hash): print("❌ Integrity check failed.", file=sys.stderr); sys.exit(4)
